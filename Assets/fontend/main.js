...
var userTools = new Vue({
    ...
    methods: {
        ...
        loginOut(){
            ...
            this.$http.get(b2_global.home_url)
            location.reload()
        },
        out(){
            this.loginOut()
...
Vue.component('page-nav',{
    ...
    methods:{
        ...
        go(page,type,action){
            ...
            if(this.type === 'm' && this.pages <= this.cpaged) return
...
var b2ShowFilterBox = new Vue({
    ...
    methods:{
        showAc(val){
            if(val){
                window.location.hash = '#ShowfilterBox';
                this.show = true
                bodyScrollLock.lock(this.$refs.filterContent)
            }else{
                this.show = false
                window.history.back()
                bodyScrollLock.unlock(this.$refs.filterContent)
            }
        },
        goBack(e){
            if(e.target.location.hash == '#ShowfilterBox'){
                this.show = true
                bodyScrollLock.lock(this.$refs.filterContent)
            }else{
                this.show = false
...
function b2scroll(){
    let allow_scroll_header = true

    let banner = document.querySelector('.header-banner-content')

    let sh = 77
    let hc = 44

    if(B2ClientWidth > 768){
        allow_scroll_header = b2_global.top_scroll_pc != '3' && b2_global.top_scroll_pc != '2'
    }else{
        sh = 62
        hc = 33
        allow_scroll_header = b2_global.top_scroll_mobile != '3' && b2_global.top_scroll_mobile != '2'
    }
    if(!banner){
        hc = 1
    }

    let outPage = document.querySelector('body.write')

    if(outPage){
        allow_scroll_header = false
    }
    
    if(b2_global.top_scroll_pc == '1' && b2AsideBar.$refs.asideContent && B2ClientWidth > 768){
        b2AsideBar.$refs.asideContent.style.transition = 'padding .3s'
    }

    let head =  document.querySelector('.header')

    let refOffset = 0;
    let bannerWrapper = head ? head.querySelector('.top-style') : false

    const footer = b2mobileFooterMenu.$refs.footerMenu;

    const asidebarTop = (offset,none)=>{
        if(!b2AsideBar.$refs.asideContent || B2ClientWidth < 768) return;
        if(none){
            b2AsideBar.$refs.asideContent.style.paddingTop = none+'px'
        }else{
            if(bannerWrapper){
                b2AsideBar.$refs.asideContent.style.paddingTop = bannerWrapper.offsetTop + sh + offset +'px';
            }
        }
    }
    ...
    const handler = () => {
        ...
        if(newOffset < hc){
            asidebarTop(hc-newOffset)
            if(allow_scroll_header && bannerWrapper){
                bannerWrapper.classList.remove('animateIn');
            }

            if(((b2_global.top_scroll_pc == '2' && B2ClientWidth > 768 && !outPage) || (b2_global.top_scroll_mobile == '2' && B2ClientWidth <= 768 && !outPage)) && bannerWrapper){
                bannerWrapper.classList.remove('banner-fixed')
            }
            
        }else{
            if(!allow_scroll_header){
                asidebarTop(0,10)
            }
            ...
            if (newOffset > refOffset) {
                ...
                if(footer && b2_global.footer_scroll == '1' && B2ClientWidth <= 768){
                    footer.classList.remove('animateIn');
                    footer.classList.add('animateOut');
                }
                ...
                if(footer && b2_global.footer_scroll == '1' && B2ClientWidth <= 768){
                    footer.classList.add('animateIn');
                    footer.classList.remove('animateOut');
                }
            }

            if(((b2_global.top_scroll_pc == '2' && B2ClientWidth > 768 && !outPage) || (b2_global.top_scroll_mobile == '2' && B2ClientWidth <= 768 && !outPage)) && bannerWrapper){
                if(b2AsideBar.$refs.asideContent && B2ClientWidth > 768){
                    b2AsideBar.$refs.asideContent.style.paddingTop = sh+'px';
                }
                
                bannerWrapper.classList.add('banner-fixed')
            }
...
function b2HiddenFooter(){
    let footer = document.querySelector('.site-footer .wrapper')
    if(!footer) return
...
function isChrome70() {
    ...
    if (test) {
        if(parseInt(test[1], 10) <= 70){
            var styleTag = document.createElement ("style");
            var head = document.getElementsByTagName ("head")[0];
            head.appendChild (styleTag);
            styleTag.sheet.addRule('.box-in {-webkit-box-shadow:inset 0px 0px 67px -34px rgba(0,0,0,.27)}.item-in {-webkit-box-shadow:inset 0px 0px 67px -34px rgba(0,0,0,.27)}', 0);
        }
    }
    return false;
}
